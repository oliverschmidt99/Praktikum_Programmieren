...
int main(void) {
    ...
    ausgabe_rat(empty,'z',zaehler_kalkulation);
        ...
        do{
            ...
            rational *zahl_1 = number_drop(zaehler_start);
            rational *zahl_2 = number_drop(zaehler_dowhile+start);
            
            if(zahl_1->nenner != 0 && zahl_2->nenner != 0){

                if(zahl_1->zaehler != 0 && zahl_2->zaehler != 0){
                rational *zahl_1 = ggT_teiler(zahl_1);         
                rational *zahl_2 = ggT_teiler(zahl_2);
                                
                rational *erg = multiplikation_rat(zahl_1, zahl_2);
                ausgabe_rat(erg, '*',0);

                rational *erg = divison_rat(zahl_1, zahl_2);
                ausgabe_rat(erg, '/',0);

                rational *erg = addion_rat(zahl_1, zahl_2);
                ausgabe_rat(erg, '+',0);

                rational *erg = subtrahtion_rat(zahl_1, zahl_2);
                ausgabe_rat(erg, '-',0);
                }
                else{
                    if(zahl_1->zaehler == 0){
